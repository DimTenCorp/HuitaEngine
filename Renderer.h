...
private:
    ...
    struct ShadowAtlas {
        ...
        GLuint texture = 0;
        int size = 2048;
        int tileSize = 768;  // 3x2 ����� �� 256 = 768x512, �� ������� 768x768
        int maxLights = 0;
        int facesPerRow = 0;
...
float sampleShadowAtlas(int slot, vec3 fragPos, vec3 lightPos, float radius) {
    ...
    // Нормализуем в [0,1] — depth-шейдер записывает length(fragPos - lightPos) / radius
    currentDepth = currentDepth / radius;
    ...
    // Bias масштабируется вместе с нормализованной глубиной [0,1]
    float bias = 0.005;
