...
// ИСПРАВЛЕННЫЙ depth шейдер: линейная глубина (расстояние / radius) для корректного сравнения
...
void main() {
    // Записываем линейную глубину в [0,1] для корректного сравнения с length(lightToFrag)/radius
...
void ShadowSystem::renderFaceToAtlas(PointLight* light, int face,
    ...
    // Scissor ограничивает glClear только тайлом текущей грани, иначе очищается весь атлас
    glEnable(GL_SCISSOR_TEST);
    glScissor(atlasX, atlasY, tileSize, tileSize);
    ...
    GLint lightPosLoc = glGetUniformLocation(depthProgram, "lightPos");
    GLint farPlaneLoc = glGetUniformLocation(depthProgram, "farPlane");

    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(glm::mat4(1.0f)));
    glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));
    glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(proj));
    glUniform3fv(lightPosLoc, 1, glm::value_ptr(light->getPosition()));
    glUniform1f(farPlaneLoc, light->radius);
...
void ShadowSystem::renderShadowMaps(const std::vector<PointLight*>& lights,
    ...
    for (auto* light : nearbyLights) {
        ...
        glUniform1f(farPlaneLoc, radius);
