...
// Статические шейдеры
const char* Renderer::getGeometryVert() {
    return R"(
#version 330 core
...
const char* Renderer::getGeometryFrag() {
    return R"(
#version 330 core
in vec2 vTexCoord;
in vec3 vNormal;
...
const char* Renderer::getLightingVert() {
    return R"(
#version 330 core
...
const char* Renderer::getLightingFrag() {
    return R"(
#version 330 core
in vec2 TexCoord;
out vec4 FragColor;
...
void main() {
    ...
    vec3 halfwayDir = normalize(-uSunDir + viewDir);
    float spec = pow(max(dot(normal, halfwayDir), 0.0), 32.0);
    vec3 specular = spec * uSunColor * 0.3;
    
    vec3 result = (diffuse + specular) * albedo.rgb;
