...
// Максимальный размер одной грани (увеличено для поддержки детальных карт с большими лайтмапами)
// Значение достаточно большое, чтобы вместить любые разумные размеры из Half-Life карт
constexpr int MAX_LIGHTMAP_SIZE = 4096;

// Максимальный размер атласа (ограничение видеокарты, обычно 16384 или даже 32768 на современных GPU)
constexpr int MAX_ATLAS_SIZE = 32768;
...
void LightmapManager::calculateOptimalAtlasSize(const std::vector<FaceLightmap>& tempMaps, int& outWidth, int& outHeight) {
    ...
    // Теперь мы НЕ округляем до степени двойки - используем точные размеры для поддержки любых разрешений
    
    // Просто берем вычисленные размеры, без округления до степеней двойки
    outWidth = w;
    outHeight = h;
